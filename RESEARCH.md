...
The same architecture — pin attention/router/embeddings, stream experts from NVMe — scales to any MoE model on any hardware where `pinned_size < available_RAM`.
...
### Multi-Machine Latency Model

The multi-machine setup introduces new latency components. Per-token latency
is the max over machines of compute + expert loads on cache miss + activation
transfer + barrier sync. The dominant term is measurable, not subtle:

- Activation per layer: hidden_size x sizeof(float16) = 2048 x 2 = **4 KB**
- Expert load per layer: 8 x 1.69 MB = **13.5 MB**
- Ratio: expert loading moves **3,375x** more data than activation transfer

So interconnect latency for activations is negligible against SSD reads, and
the cluster feasibility condition reduces to
`N * S * cache_hit_rate > bytes_per_token * target_tok_s`
(N machines, S = per-machine SSD bandwidth) — a concrete engineering target
for how many machines a given speed requires.

## Files (this repo)

| Path | Purpose |
|------|---------|
| `src/mlx_expert_sniper/` | MLX package: expert reader, engines, calibrate, eval, CLI, server |
| `llama-cpp/patches/` | Expert-cache sources + apply.sh for a stock llama.cpp checkout |
| `llama-cpp/sniper.py` | Interactive agent (chat, search, vision, shell) |
| `sniper-router/router.py` | Remote inference client |
| `bench/` | Research benchmark scripts behind the numbers in this document |
