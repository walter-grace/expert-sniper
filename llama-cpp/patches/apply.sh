#!/bin/bash
# Apply Expert Sniper patches to a stock llama.cpp checkout.
#
# Usage:  ./apply.sh /path/to/llama.cpp
#
# 1. Copies the expert-cache sources (original Expert Sniper code, MIT)
#    into llama.cpp/src/
# 2. Anchor-patches CMakeLists.txt, common/common.h, common/arg.cpp,
#    common/common.cpp (idempotent — safe to re-run; warns and skips if
#    an anchor moved upstream)
#
# Tested against llama.cpp master as of 2026-08. The patcher is
# version-tolerant by design; if a [WARN] anchor is reported, apply that
# hunk by hand (each is a few lines).
set -e

PATCH_DIR="$(cd "$(dirname "$0")" && pwd)"
LLAMA_DIR="${1:?usage: ./apply.sh /path/to/llama.cpp}"
cd "$LLAMA_DIR"

echo "Copying expert cache sources into $LLAMA_DIR/src/"
cp "$PATCH_DIR"/src/llama-expert-cache*.cpp "$PATCH_DIR"/src/llama-expert-cache*.h src/
